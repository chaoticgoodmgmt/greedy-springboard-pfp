(function() {
    ...
    function Button(channel, className, text, label) {
        if (!label) label = '';
        if (!text) text = '';
        var button = $('<div class="button-container ' + className + '">' + label + '<div class="button-control">' + text + '</div></div>');
    ...
    function Fader(channel, count, className) {
        var fader = $('<div class="fader-container ' + className + '"><div class="channel-notches"><div class="channel-notch"></div><div class="channel-notch"></div><div class="channel-notch zeroed">0</div><div class="channel-notch"></div><div class="channel-notch"></div><div class="channel-notch"></div><div class="channel-notch"></div><div class="channel-notch"></div><div class="channel-notch"></div><div class="channel-notch"></div></div><div class="fader-no">' + count + '</div><div class="fader-track"><div class="fader"><div></div></div></div></div>');
        channel.append(fader);
        $(fader).find('.fader').on('mousedown', function(e) {
            var el = $(this), pos = e.pageY, offset = el.position().top;
    ...
    function RotaryKnob(channel, label, className) {
        ...
        $(knobTemplate).find('.dial').on('mousedown', function(e) {
            var el = $(this), offset = el.offset(), center_x, center_y, mouse_x, radians, degree, degreeRatio;
            $(document).mousemove(function(evt) {
    ...
    Daw.prototype.updateLoader = function(current) {
        ...
        console.log("Updated loader: ", current + 1);
    }
    ...
    function Mixer(ctx, tracks) {
        ...
        var track, trackName, trackBuffer, count = 0, rawBuffer;
        this.soloed = 0;
        this.channels = [];
        this.el = $('#mixer');
        this.ctx = ctx;
        this.duration = 200; // hardcode duration for time being as the tracks are longer than the song is audible
        this.tracks = tracks;
        ...
        for (var i = 0; i < tracks.length; ++i) {
            ...
            console.log("Creating channel for track:", trackName);
            this.updateDuration(buffSource.buffer.duration * 1000, trackName);
            this.channels.push(new Channel(rawBuffer, buffSource, trackName, this, ctx, i));
        }
    ...
    Mixer.prototype.createMasterChannel = function() {
        var masterChannel = $(channelTemplate), masterFader = new Fader(masterChannel, '&nbsp;', 'master'), oldMin = 285, oldMax = 0, newMin = 0, newMax = 1.3, newVol, self = this;
        masterChannel.addClass('master-channel');
        this.masterGain = this.ctx.createGain();
        this.masterGain.gain.value = 1;
        this.el.append(masterChannel);
        masterFader.on('fader', function(e, val) {
            newVol = ((val - oldMin) / (newMin - oldMin)) * (newMax - newMin) + newMin;
    ...
    Channel.prototype.createChannelLabel = function() {
        ...
        this.el.find('.fader-container').prepend('<p class="label">' + trackName + '</p>');
    }
        ...
        for (i = 0; i < this.mixer.channels.length; ++i) {
            if (!this.mixer.channels[i].on) {
                ...
                if (this.mixer.soloed === 0) {
                    this.mixer.channels[i].gain.gain.value = this.mixer.channels[i].currGain;
    ...
    Channel.prototype.createChannelControls = function() {
        ...
        this.faderControl.on('fader', function(e, val) {
            var oldMin = 295, oldMax = 0, newMin = 0, newMax = 1.3;
            var newVol = ((val - oldMin) / (newMin - oldMin)) * (newMax - newMin) + newMin;
